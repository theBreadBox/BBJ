...
function Mint() {
	...
	const mintNFTHandler = async () => {
		if (nft) {
			...
			await nft.methods.publicMint(amtToMint).send({ from: account, value: (amtToMint*Price)})
	...
	const presaleMintNFTHandler = async () => {
		// Presale Mint NFT
		if (nft) {
			await nft.methods.presaleMint(amtToMint, mvpProof).send({ from: account, value: (amtToMint*Price) })
				.on('confirmation', async () => {
					const totalSupply = await nft.methods.totalSupply().call()
					setTotalSupply(totalSupply)
				})
	...
	const renderHandleButton = () => {
		if (!account) {
			...
			return <GridCol sm={12} md={6}>
				<MintLabel for="amtToMint">How many?</MintLabel><InputWrapper><InputArrows onClick={decreaseClick}>-</InputArrows><MintInput onChange={handleInput} type="number" name="amtToMint" id="amtToMint" max="100" min="1" value={inputValue}/><InputArrows onClick={increaseClick}>+</InputArrows></InputWrapper>
	...
	const renderSupplyAvailable = () => {
		if (!account) {
			...
			return <AmountMinted>{totalSupply}/{maxSupply} minted!</AmountMinted>
		}
	}
